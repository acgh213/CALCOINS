...
const string strMessageMagic = "CalCoins Signed Message:\n";
...
{
    int64 nSubsidy = 611 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 12 * 60 * 60; // CalCoins: 1 days
static const int64 nTargetSpacing = 15; // CalCoins: .5 minute blocks
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CalCoins", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "SpaceShipTwo blew up and investigation";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1414955961 ; //epochtimeon2014112
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0;

        if (fTestNet)
        {
            block.nTime    = 1414955961 ;
            block.nNonce   = 0;
        }
